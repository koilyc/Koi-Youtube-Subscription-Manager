  ...
  static #renderList(channel: Channel, callbacks: ChannelCallbacks, folders: Folder[]): HTMLElement {
    ...
    el.innerHTML = `
      ...
      <div class="channel-item__actions">
        ...
        <button class="btn btn--icon btn--unsubscribe" data-action="unsubscribe" title="Unsubscribe">
    ...
    const unsubBtn = el.querySelector<HTMLButtonElement>('[data-action="unsubscribe"]');
    if (unsubBtn) {
      unsubBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        callbacks.onUnsubscribe?.(channel.id, channel.name);
      });
    }
